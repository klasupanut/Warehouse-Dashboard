...
OUT = ROOT / "chaengwattana-plan.svg"
...
def styled_stroke(source_color, source_width):
    if source_color == "#000000":
        if source_width >= 0.9:
            return "#dffaff", "0.48", 0.8
        return "#dffaff", "0.32", 0.68
    if source_color in ("#ed1f24", "#cd2027"):
        return "#dffaff", "0.88", 1.05
    if source_color == "#93278f":
        return "#dffaff", "0.72", 0.98
    if source_color == "#939598":
        return "#b8d9e5", "0.42", 0.72
    if source_color == "#f8991e":
        return "#ffd166", "0.32", 0.72
    return "#dffaff", "0.36", 0.72


def styled_fill(source_color):
    if source_color in ("#fafafa", "#ffffff"):
        return "#ffffff", "0"
    if source_color in ("#a7bc8a", "#5d9b55"):
        return "#2fffc7", "0.08"
    if source_color == "#dbdbdb":
        return "#dffaff", "0.035"
    return source_color, "0.04"
        ...
        if operator == "q":
            ...
            if path_cmds:
                ...
                elements.append(
                    ...
                    f'stroke="{stroke}" stroke-width="{fmt(max(0.36, state["width"] * width_scale))}" '
